        ...
        if self.n_components == -1:
            ...
            if self.print_more:
                if self.method == "PCA":
                    ...
                    print("UMAP fit data. dim = {} and #data = {}.".format(self.n_components, data.shape))
...
class ConsistencyMethod(object):
    ...
    def transform(self, data: list, theta_np=None):
        ...
        z0, z1 = torch.tensor(self.add_ones_dimension(data[0]).dot(theta_np.T)), torch.tensor(
            self.add_ones_dimension(data[1]).dot(theta_np.T)
        )
    ...
    def fit(self, data: list, label, nepochs=1000, ntries=10, lr=1e-2, init_theta=None, device="cuda"):
        ...
        if self.verbose:
            print("String fiting data with Prob. nepochs: {}, ntries: {}, lr: {}".format(nepochs, ntries, lr))
...
class myClassifyModel(LogisticRegression):
    ...
    def fit(self, data, label, times=20, use_scheduler=False, weights=None, lr=1e-1, epochs=20, device="cuda"):
        if self.method == "LR":
            ...
            assert data.shape[1] == 1, "When `avg` mode is used, #hidden_dim is expected to be 1, but it's {}".format(
                data.shape[1]
            )
            self.avg = 0.0
            self.sign = 1
            ...
            for _ in range(times):
                ...
                for epoch in range(epochs):
                    ...
                    if ((epoch + 1) % 50 == 0 and self.print_more) or epoch in [0, epochs - 1]:
                        ...
                        projected, gth = np.concatenate([w @ theta_np.T for w in data]).reshape(-1), np.concatenate(
                            label
                        ).reshape(-1)
                        ...
                        if self.print_more:
                            print(
                                "update params, acc is {:.2f}, old loss is {:.4f}, new loss is {:.4f}".format(
                                    100
                                    * self.score(np.concatenate(data, axis=0), np.concatenate(label), sample_weight),
                                    minloss,
                                    loss,
                                )
                            )
